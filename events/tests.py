...
class ViewTest(TestCase):
    def setUp(self):
        self.ev1 = Event.objects.create(when=now, what="ev1")
        self.ev2 = Event.objects.create(when=now + timedelta(days=1, seconds=2*3600+34*60), 
    ...
    def testDetailView(self):
        c= Client()
        res = c.get(self.ev2.get_absolute_url())
        self.assertEquals(res.context['in_days'], 1)
        self.assertEquals(res.context['in_hours'], 2)
        #self.assertEquals(res.context['in_minutes'], 33)
